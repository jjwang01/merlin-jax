...
class Agent():
    def __init__(self, lstm_width_1, lstm_width_2, latent_size, num_mems, num_actions, policy_hidden_size=200, activation='tanh', **kwargs):
    ...
    def call_mbp_lstm(self, x):
        ...
        fx_project = keras.layers.Dense(self.num_mems * (2 * self.latent_size + 1), activation=self.activation)(fx_lstm)
        fx_reshaped = keras.layers.Reshape(shape=(self.num_mems, 2 * self.latent_size + 1))(fx_project)
    ...
    def write(self, z_t, discount):
        ...
        self.memory = self.memory + jnp.matmul(
            write_weight, 
            jnp.transpose(
                jnp.concatenate((z_t, jnp.zeros(self.latent_size))))
        ) + jnp.matmul(
            self.retro_weight, 
            jnp.transpose(
                jnp.concatenate((jnp.zeros(self.latent_size), z_t)))
        )
    ...
    def prior(self, h_prev, m_prev):
        ...
        for _ in range(2):
            res = keras.layers.Dense(2 * self.latent_size, activation=self.activation)(res)
    ...
    def posterior(self, e_t, h_prev, m_prev, mu_prior, log_sigma_prior):
        ...
        for _ in range(2):
            res = keras.layers.Dense(2 * self.latent_size, activation=self.activation)(res)
        return keras.layers.Dense(2 * self.latent_size)(res) + jnp.concatenate((h_prev, m_prev))

    # sample from posterior
    def generate_state_variable(self, mu_post, log_sigma_post):
        return mu_post + jnp.dot(jnp.exp(log_sigma_post), jax.random.normal(shape=(self.latent_size)))
    ...
    def act(self, z_t):
        ...
        # 2 * |z| + 1 -> h_t, m_t
        fx_project = keras.layers.Dense(2 * self.latent_size, activation=self.activation)(fx_lstm)
        fx = jnp.concatenate((z_t, fx_project))
        mlp_output = keras.layers.Dense(self.policy_hidden_size, activation=self.activation)(fx)
